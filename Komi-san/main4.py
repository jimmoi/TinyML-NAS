...
def load_data_kfold(data_dir, input_shape, batch_size, k_folds=5, cache=True):
    ...
    # 3. เริ่มลูปการแบ่งข้อมูล
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(all_image_paths)):
        
        # --- เพิ่มขั้นตอน: แบ่ง temp_val_idx ออกเป็น Validation และ Test ---

        # 4. สร้าง Dataset สำหรับทั้ง 3 ส่วน
        train_ds = make_dataset(all_image_paths[train_idx], all_labels[train_idx],
