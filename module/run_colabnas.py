...
from pso_nas_structured import NASPsoOptimizer, ArchitectureSearchSpace, ModelDecoder
...
def main():
    ...
    # Choose NAS algorithm
    print(f"\n🧠 Available NAS algorithms:")
    print("  1. OurNAS (Original algorithm)")
    print("  2. PSONAS (Simple PSO)")
    print("  3. Structured PSO (Modular components)")
    
    while True:
        choice = input("\nSelect algorithm (1, 2, or 3): ").strip()
        if choice in ['1', '2', '3']:
            break
        print("Please enter 1, 2, or 3")
    
    if choice == "2":
        print("🔬 Using simple PSO-based NAS...")
        nas_algorithm = PSONAS
    elif choice == "3":
        print("🧬 Using structured PSO-based NAS...")
        # Setup structured PSO components
        search_space = ArchitectureSearchSpace(k_range=(4, 32), c_range=(0, 5))
        decoder = ModelDecoder()
        nas_algorithm = NASPsoOptimizer.setup(
            search_space=search_space,
            decoder=decoder,
            n_particles=5,
            iterations=10
        )
        print(f"  Particles: {nas_algorithm.n_particles}, Iterations: {nas_algorithm.iterations}")
    else:
        print("🔬 Using original NAS algorithm...")
        nas_algorithm = OurNAS
    ...
    try:
        path_to_tflite_model = colabNAS.search(nas_algorithm)
